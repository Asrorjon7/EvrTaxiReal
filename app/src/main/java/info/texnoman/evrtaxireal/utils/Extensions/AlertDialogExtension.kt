...
class AlertDialogHelper(context: Context, title: CharSequence?, message: CharSequence?) {
    ...
    private val backTo:Button by lazyFast {
        dialogView.findViewById<Button>(R.id.backto)
    }
    ...
    fun backto(text: CharSequence, func: (() -> Unit)? = null) {
        with(backTo) {
            this.text = text
            setClickListenerToDialogButton(func)
        }
